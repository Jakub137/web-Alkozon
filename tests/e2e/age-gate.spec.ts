...
test.describe("Age Gate Restrictions", () => {
  test("powinien zablokować dodanie zamówienia własnego dla osoby niepełnoletniej", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("Ta część strony zawiera ofertę produktów alkoholowych")
    ).toBeVisible();
  });
