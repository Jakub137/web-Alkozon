...
test.describe('UI Preferences', () => {
  test('powinien przełączyć język na angielski', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: 'Discover the world of premium spirits' })).toBeVisible();
  });
  ...
  test('powinien przełączyć i zapamiętać dark mode po odświeżeniu', async ({ page }) => {
    ...
    const themeToggle = page.getByRole('button').filter({ hasText: /Tryb|Mode/ }).first();
