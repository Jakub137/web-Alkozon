...
describe('RegisterPage Unit Tests', () => {
  ...
  it('powinien pokazać błędy walidacji dla pustego formularza', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Musisz potwierdzić pełnoletność, aby założyć konto klienta.')).toBeInTheDocument();
    });
  });
  ...
  it('powinien poprawnie zarejestrować przy prawidłowych danych', async () => {
    ...
    fireEvent.click(screen.getByLabelText('Potwierdzam, że mam ukończone 18 lat i mogę założyć konto klienta.'));
