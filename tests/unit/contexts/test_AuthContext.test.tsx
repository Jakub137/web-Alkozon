...
describe("AuthContext Unit Tests", () => {
  ...
  it("funkcja login powinna poprawnie ustawić token i usera oraz zapisać do localStorage", () => {
    ...
    expect(JSON.parse(localStorage.getItem("alkozon_auth_session") || "{}").accessToken).toBe(
      "fake-token"
    );
    expect(JSON.parse(localStorage.getItem("alkozon_auth_session") || "{}").user.username).toBe(
      "testuser"
    );
  });
